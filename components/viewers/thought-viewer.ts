...
class ThoughtViewer extends HTMLElement {
    ...
    private _data?: ThoughtData;
    private page: number = 0;
    private spreadIndex: number = 0;
    private _onKey?: (e: KeyboardEvent) => void;
    private _onResize?: () => void;
    private _arrows?: HTMLElement;
    private _onArrowsPrev?: () => void;
    private _onArrowsNext?: () => void;
    ...
    set data(v: ThoughtData) { this._data = v; this.page = 0; this.spreadIndex = 0; this.render(); }
    ...
    `;
        ...
        const arrows = this.shadowRoot.querySelector('navigation-arrows') as HTMLElement;
        if (arrows) {
    ...
    private _hasMoreSpreads(): boolean {
        if (!this.shadowRoot) return false;
        const track = this.shadowRoot.querySelector('.pages-container') as HTMLElement;
    ...
    private _updateArrows(arrows: HTMLElement | null): void {
        if (!arrows) return;
        const thoughts = this._data?.thoughts || [];
    ...
    prev(): void {
        if (this.spreadIndex > 0) {
            ...
            this.updateSpread();
            if (this.shadowRoot) {
                this._updateArrows(this.shadowRoot.querySelector('navigation-arrows') as HTMLElement); // update State
            }
        } else if (this.page > 0) {
            this.page--;
            this.spreadIndex = 0;
    ...
    next(): void {
        ...
        if (currentPos < track.scrollWidth - 5) {
            this.spreadIndex++;
            this.updateSpread();
            this._updateArrows(this.shadowRoot.querySelector('navigation-arrows') as HTMLElement);
        } else {
            // Next thought
            const thoughts = this._data?.thoughts || [];
